...
export default function Reels() {
  ...
  const { data: userInfo, isLoading, error, isError } = useSiteData()
      ...
      <main className="reels-main">
        ...
        <div className="reels-card">
          <div className="reels-toolbar">
            ...
            <button className="reels-btn reels-btn-secondary">
              <Filter size={16} />
              Filters
            </button>
          </div>
          ...
          {isLibraryLoading ? (
            ...
            <div className="reels-grid">
              {visibleReels?.map((reel) => (
                <div
                  key={reel._id}
                  className="reel-item-card"
                  onClick={() => navigate(`/reels/edit/${reel._id}`)}
                >
                  <div className="reel-thumb-container">

                    {/* Thumbnail Logic: Use thumbnailUrl for videos, url for images */}
                    ...
                    {/* {reel.processingStatus && (
                      <div className={`status-badge ${reel.processingStatus}`}>
                        {reel.processingStatus === 'processing' && <Loader2 size={12} className="spin" />}
                        {reel.processingStatus}
                      </div>
                    )} */}

                    {/* Type Overlay (Play icon vs Image icon) */}
